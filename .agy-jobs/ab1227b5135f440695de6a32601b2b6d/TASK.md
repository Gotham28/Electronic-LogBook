...
**Do not call `run_command` or any shell tool at all, for any reason, even once, even to
check git status.** Every shell command in this environment gets denied and the task must
continue anyway using file-reading/file-editing tools only (`view_file`, `list_dir`,
`grep_search`, `replace_file_content`, `write_to_file`). If you are tempted to run an
orientation command like `git status`, do not — skip straight to the file-based steps below.
...
Any database command. `git commit`/`push`/PR. Any shell command whatsoever.
