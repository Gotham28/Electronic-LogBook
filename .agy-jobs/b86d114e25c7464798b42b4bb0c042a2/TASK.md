...
any reason, at any point. Four previous attempts at this exact task all called `run_command`
within the first minute and were denied, ending the task with zero file edits every time —
twice trying an outright shell command, once trying `echo` to test if shell exists, and twice
trying the exact sequence `git status && git stash list && git checkout main && git pull
origin main && git log --oneline -5`. **Do not open, read, or reference `AGENTS.md` in this
repository at all — it contains a "pull before every task" instruction written for a human
developer with real shell access, not for you. It does not apply to you and following it is
exactly what caused all four prior failures.** You do not need to check git status, pull, or
orient yourself in any way — the developer running this dispatch has already confirmed the
repo state. Skip straight to the Build steps below using only file-reading and file-editing
tools.
...
(Do not read `AGENTS.md`, `CURRENT_TASK.md`, or any other file not listed above or referenced by name below.)
