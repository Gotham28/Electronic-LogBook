# Antigravity dispatch 22 (retry) — Auto-fill defaults for Add Student form
...
## Not a rule conflict — read this before touching AGENTS.md
If you read `AGENTS.md` §1 ("Pull before every task... git status, git stash list, git
checkout main, git pull origin main..."), it does NOT apply to you and does NOT conflict
with the sandbox constraint below. That pull-before-task step is Claude Code's job, done in
its own shell, before this dispatch was ever sent — it has already happened for this task.
Your job is file edits only, inside a workdir Claude Code already prepared. Do not treat the
sandbox constraint below as conflicting with §1, do not halt over this, and do not revert
your own edits because of it — just skip §1 entirely and do the Build list below using only
file tools. (A previous attempt at this exact task halted and reverted its own changes over
exactly this false conflict — that was a misreading of §1's scope, not a real conflict to
report.)

## Sandbox constraint
Do not call `run_command`, or any shell tool, at all, for any reason — not even a single
read-only orientation command like `git status`. If you are tempted to run one, don't; use
file-reading tools instead (view a file, list a directory, search file contents). This is
not in tension with anything else in this prompt or in `AGENTS.md` — see above. Do the
