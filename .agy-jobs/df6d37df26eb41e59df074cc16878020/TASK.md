...
any reason, at any point — not to check your work, not to look something up, not even once.
The previous two attempts at this exact task both tried to run a shell command within the
first few seconds and were denied, ending the task with zero progress before a single file
was even read. Do not repeat that. Every single thing you need — including how to figure out
the correct expected numbers for new test assertions — must be done by reading files with
your file tool and reasoning about what you read, never by running anything.
