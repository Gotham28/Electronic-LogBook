...
| SEC-38 | Medium | `artifacts/api-server/package.json` → `nodemailer` | Production dependency added in commit `b4b3233` (2026-09-10) carries five advisories: 1 high-severity (GHSA-2x7j-588g-ccc2: quadratic-time complexity in `addressparser`, remote DoS via crafted address lists) and 4 moderate-severity (GHSA-8m3c-c648-2xjj: `resolveContent()` bypass of file/URL access restrictions under legacy signature; GHSA-wmmp-3585-3rmp: IDN/Punycode domain allow-list bypass; GHSA-cc9r-2j5m-2m83: recipient-domain validation bypass via RFC 5322 comment mis-parsing; and related IDN domain validation advisory). | Indirect exposure on account-creation email delivery. Rated Medium matching SEC-21's convention for indirect dependency vulnerabilities without direct unauthenticated exploit paths on application routes. | Upgrade `nodemailer` to `>=6.10.1` (or latest patched release) — a dependency update requiring explicit developer decision. |

### Low

| ID | Sev | File:line | What is wrong | What an attacker or wrong user could actually reach | Proposed fix |
|---|---|---|---|---|---|
| SEC-22 | Low | `artifacts/api-server/src/routes/department.ts:106-118` | The analytics student query filters on `usersTable.departmentId` but not `usersTable.status`, unlike the roster query at `department.ts:60-80`. | Rejected and pending students are counted in `totalStudents` and appear in the HOD's registrations table as "Active". Same department only — no cross-tenant exposure. | Add `eq(usersTable.status, "approved")`. |
