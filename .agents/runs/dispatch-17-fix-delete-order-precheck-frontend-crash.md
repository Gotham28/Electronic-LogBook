...
## Model
Claude Opus 4.6 (Thinking) if available; if the Claude tiers are still quota-limited on
this account, use Gemini 3.1 Pro (High) instead — this fix round includes new query/design
work (the pre-check), not just a one-line correction, and touches the same
ownership-adjacent hard-delete logic that triggered Opus tier originally.
