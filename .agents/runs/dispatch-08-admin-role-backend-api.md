...
- [ ] Confirm `/auth/register` cannot create an `admin`-role account. If the existing
      registration schema doesn't already reject it, add the restriction there is not part
      of this task's scope to modify beyond adding a test that proves current behavior
      already rejects it — if it does NOT already reject it, stop and report this as a
      finding rather than editing `routes/auth.ts` yourself (that file is not in this
      dispatch's Build list).
